"""Script para explorar a tabela TGFCAB."""
import asyncio
import json
...
async def explore():
    ...
    # 3. Domínios
    campos_dominio = ['TIPMOV', 'STATUSNOTA', 'STATUSNFE', 'PENDENTE', 'CONFIRMADA']
    results["dominios"] = {}
    ...
    # 4. Sample
    sql_sample = """
        SELECT NUNOTA, NUMNOTA, DTNEG, CODPARC, CODTIPOPER, TIPMOV, STATUSNOTA, VLRNOTA, CODEMP
        FROM TGFCAB
        WHERE ROWNUM <= 5
    """
    ...
    # 5. Contagem
    results["contagem"] = await sankhya.execute_query("SELECT COUNT(*) as total FROM TGFCAB")

    # Salva em JSON
    with open("tgfcab_data.json", "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    print("Dados salvos em tgfcab_data.json")
