"""Script para explorar a tabela TGFITE."""
import asyncio
import json
...
async def explore():
    ...
    sql_chaves = """
        ...
        WHERE uc.TABLE_NAME = 'TGFITE'
            AND uc.CONSTRAINT_TYPE IN ('P', 'R')
        ORDER BY tipo_chave, campo
    """
    results["chaves"] = await sankhya.execute_query(sql_chaves)

    # 3. Domínios - campos importantes
    campos_dominio = ['USOPROD', 'ATUALESTOQUE', 'RESERVADO', 'STATUSNOTA']
    results["dominios"] = {}
    ...
    # 4. Sample com dados principais
    sql_sample = """
        SELECT NUNOTA, SEQUENCIA, CODPROD, QTDNEG, VLRUNIT, VLRTOT, VLRDESC, CODVOL, CODLOCALORIG, ATUALESTOQUE
        FROM TGFITE
        WHERE ROWNUM <= 10
    """
    ...
    # 5. Contagem
    results["contagem"] = await sankhya.execute_query("SELECT COUNT(*) as total FROM TGFITE")

    # 6. Exemplo de join com TGFCAB
    sql_join = """
        SELECT
        ...
        WHERE c.TIPMOV = 'V' AND ROWNUM <= 5
    """
    results["join_cab"] = await sankhya.execute_query(sql_join)

    # Salva em JSON
    with open("tgfite_data.json", "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    print("Dados salvos em tgfite_data.json")
