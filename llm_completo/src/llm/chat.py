...
GROQ_MODEL = os.getenv("LLM_MODEL", "llama-3.3-70b-versatile")  # Configurável via .env
...
class KnowledgeBase:
    ...
    def load(self):
        ...
        console.print(f"[system]📚 Base carregada: {len(self.documents)} documentos[/]")
        self._build_index()
    ...
    def _build_index(self):
        ...
        try:
            ...
            console.print("[system]🔍 Índice de busca criado (TF-IDF)[/]")

        except ImportError:
            console.print("[system]⚠️  scikit-learn não instalado. Usando busca por keywords.[/]")
            self.vectorizer = None
...
class ChatEngine:
    ...
    def initialize(self):
        ...
        console.print(Panel(
            ...
            "Seu assistente inteligente sobre o negócio MMarra",
            border_style="cyan",
        ))
        ...
        for cat, count in sorted(summary.items()):
            emoji = {"tabela": "📋", "processo": "🔄", "regra": "⚖️", "query": "🔍", "api": "🔌"}.get(cat, "📄")
            console.print(f"  {emoji} {cat}: {count} documentos")

        # Verificar Groq
        if not GROQ_API_KEY:
            console.print("[error]❌ GROQ_API_KEY não configurada no .env[/]")
            sys.exit(1)
        ...
        try:
            ...
            console.print(f"[system]✅ Groq conectado ({GROQ_MODEL})[/]")
        except Exception as e:
            console.print(f"[error]❌ Erro ao conectar Groq: {e}[/]")
            sys.exit(1)

        # Calcular tamanho da base
        total_chars = sum(len(d["content"]) for d in self.kb.documents)
        total_tokens_aprox = total_chars // 4
        console.print(f"[system]📊 Base total: ~{total_tokens_aprox:,} tokens[/]")

        if total_tokens_aprox < 60000:
            self.mode = "full"
            console.print("[system]📦 Modo: contexto completo (base cabe no context window)[/]")
        else:
            self.mode = "rag"
            console.print("[system]🔍 Modo: RAG (busca documentos relevantes por pergunta)[/]")
    ...
    def ask(self, question):
        ...
        try:
            ...
            return f"❌ Erro na LLM: {e}", sources_info
        ...
        while True:
            ...
            if question.lower() == "status":
                ...
                console.print(Panel(
                    ...
                    f"🔍 Modo: {self.mode}\n"
                    f"💬 Histórico: {len(self.history) // 2} trocas\n"
            ...
            if question.lower() == "limpar":
                self.history.clear()
                console.print("[system]Histórico limpo![/]")
                continue
...
def main():
    if not GROQ_API_KEY:
        console.print("[error]❌ Configure GROQ_API_KEY no arquivo .env[/]")
